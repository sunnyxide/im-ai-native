...
| 시나리오 | 결과 | 시간 |
|---|---|---|
| stub-fn — 주어진 테스트에 맞춰 빈 함수 구현 | 통과 | 28s |
...
| mutable-default — 상태가 새는 가변 기본 인자 수정 | 통과 | 26s |
...
앞선 실행에서는 10개 중 9개였습니다. 그 뒤로 검사는 세 차례의 적대적 리뷰를 거쳤고, 매번 **느슨해진 게 아니라 더 엄격해졌습니다.** 1차: "이 파일은 건드리지 말라"고 한 시나리오는 이제 그 파일이 baseline 커밋과 1바이트라도 다르면 기능 검사를 돌리기도 전에 실패 처리합니다. 이 가드는 10개 중 7개에 적용되고, 지금까지 한 번도 발동하지 않았습니다. 1차에서 `write-tests` 검사도 아무 예외나 받아주던 것을 과제가 요구한 예외 타입만 인정하도록 조였습니다.

2차는 "커버리지처럼 보이는 것"과 "진짜 커버리지"를 구분하지 못하던 구멍 두 개를 막았습니다. `write-tests`는 원래 파일 안에 `parse_port`라는 문자열만 있으면 어떤 테스트 파일이든 받아줬습니다 — 필수 케이스(정수가 아닌 입력, 범위 밖 양방향)를 하나도 안 다루는 한 줄짜리 스모크 테스트도 통과 판정을 받을 수 있었습니다. 이제는 뮤테이션 테스트로 커버리지를 증명합니다. Codex가 만든 테스트 파일을, 일부러 망가뜨린 네 가지 구현체(각각 격리된 임시 디렉터리에서) 각각에 대해 돌려서 전부 실패하는지 확인합니다. `two-file-consistency`는 원래 모든 레코드에 `checksum=0` 같은 상수 체크섬을 붙여도 통과했습니다 — Codex에게 주는 테스트가 `checksum=` 필드 존재와 라운드트립 생존만 확인했기 때문입니다. 이제 채점기가 Codex가 만든 코드를 직접 import해서, 레코드 내용이 바뀌면 체크섬도 실제로 바뀌는지, 안 바뀌면 체크섬도 그대로인지 확인합니다. 이번 실행에서는 두 신규 가드 모두 발동하지 않았습니다 — 두 검사를 배달된 파일에 대해 독립적으로 다시 돌려보고, 배달된 `port.py`/`test_port.py`와 `codec.py`를 직접 읽어서 재확인했습니다.

앞선 (9/10) 실패는 Codex의 작업이 아니라 채점기의 버그였습니다. 과제문에 테스트 프레임워크를 지정하지 않았고 Codex는 pytest 스타일로 제대로 작성했는데 채점기가 `unittest`만 돌렸습니다. 지금은 과제문이 `unittest`를 명시하므로, 그 시나리오 하나는 예전보다 덜 모호해진 것도 사실입니다. 가드에는 알아둘 만한 빈틈이 남아 있습니다. Codex가 나머지를 고쳐야 하는 파일 안에서 "이 *함수*는 건드리지 말라"고 한 약속은 파일 단위 diff로 강제할 수 없고, 뮤테이션 테스트도 과제문이 이름으로 콕 집은 행동(정상 포트, 정수 아님, 너무 낮음, 너무 높음)만 커버합니다 — 진짜 커버리지 도구를 대체하는 범용 장치가 아닙니다.
